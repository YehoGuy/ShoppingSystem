...
public class PurchaseCompletionIntermidiate extends VerticalLayout {
    ...
    private void setupAddressForm() {
        ...
        add(countryField, cityField, streetField, houseNumberField, apartmentNumberField, zipCodeField,shippingTypeCombo);
    }

    private Address getAddressFromForm() {
        return new Address()
                .withCountry(countryField.getValue())
                .withCity(cityField.getValue())
                .withStreet(streetField.getValue())
                .withHouseNumber(houseNumberField.getValue())
                .withApartmentNumber(apartmentNumberField.getValue().isEmpty() ? 0 : Integer.parseInt(apartmentNumberField.getValue()))
                .withZipCode(zipCodeField.getValue());
    }
    ...
    private void completePurchase(Address address) {
