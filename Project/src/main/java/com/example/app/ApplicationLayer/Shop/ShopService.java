...
public class ShopService {
    ...
    public List<String> getShopsNames(List<Integer> shopsIds, String token) {
        try {
            LoggerService.logMethodExecution("getShopsNames", shopsIds);
            authTokenService.ValidateToken(token);
            List<String> returnShopsNames = shopRepository.getShopsNames(shopsIds);
            LoggerService.logMethodExecutionEnd("getShopsNames", returnShopsNames);
            return returnShopsNames;
        } catch (OurArg e) {
            LoggerService.logDebug("getShopsNames", e);
            throw new OurArg("getShopsNames" + e.getMessage());
        } catch (OurRuntime e) {
            LoggerService.logDebug("getShopsNames", e);
            throw new OurRuntime("getShopsNames" + e.getMessage());
        } catch (Exception e) {
            LoggerService.logError("getShopsNames", e, shopsIds);
            throw new OurRuntime("Error retrieving shop names: " + e.getMessage(), e);
        }
    }
}
